...
/// <summary>
/// Testes unitários do GitHubCopilotProvider — propriedades e contrato de interface.
...
{
    [Fact]
    public void ProviderId_ShouldReturnGithubCopilot()
    {
        GitHubCopilotProvider.ProviderIdentifier.Should().Be("github-copilot");
    }

    [Fact]
    public void GitHubCopilotProvider_ShouldNotBeLocal()
    {
        var provider = Substitute.For<IAiProvider>();
        provider.ProviderId.Returns("github-copilot");
    ...
    public async Task ListAvailableModelsAsync_ShouldReturnStaticModelList()
    {
        ...
        models.Should().Contain(m => m.ModelId == "copilot-chat");
    }
    ...
    public async Task CheckHealthAsync_ShouldReturnUnhealthy_WhenApiNotReachable()
    {
        ...
        provider.CheckHealthAsync(Arg.Any<CancellationToken>())
            .Returns(new AiProviderHealthResult(false, "github-copilot", "Health check failed: Connection refused", TimeSpan.FromMilliseconds(5)));
    ...
    public async Task CompleteAsync_ShouldReturnFailure_WhenProviderThrows()
    {
        ...
        chatProvider.ProviderId.Returns("github-copilot");
        chatProvider.CompleteAsync(Arg.Any<ChatCompletionRequest>(), Arg.Any<CancellationToken>())
            .Returns(new ChatCompletionResult(false, null, "gpt-4o", "github-copilot", 0, 0, TimeSpan.Zero, "API error"));
