...
public sealed class LoginResponseBuilderTests
{
    ...
    {
        var user = User.CreateLocal(Email.Create("bob@example.com"), FullName.Create("Bob", "Smith"), HashedPassword.FromPlainText("P@ssw0rd123!"), DateTimeOffset.UtcNow);
        var membership = TenantMembership.Create(user.Id, TenantId.From(Guid.NewGuid()), RoleId.New(), Now);
    ...
    {
        var user = User.CreateLocal(Email.Create("charlie@example.com"), FullName.Create("Charlie", "X"), HashedPassword.FromPlainText("P@ssw0rd123!"), DateTimeOffset.UtcNow);
        var membership = TenantMembership.Create(user.Id, TenantId.From(Guid.NewGuid()), RoleId.New(), Now);
        var role = Role.CreateSystem(membership.RoleId, Role.Developer, "Dev");
