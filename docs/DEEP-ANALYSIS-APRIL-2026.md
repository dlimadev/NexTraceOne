...
| Handler | Módulo | Campos Hardcoded |
|---------|--------|------------------|
| `GetExecutiveDrillDown` | Governance | ReliabilityScore="N/A", ChangeSafety="N/A", TopGaps vazio |
| `GetAutomationValidation` | OperationalIntelligence | Retorna sempre `Array.Empty<ValidationCheckDto>()` |
| `GetAutomationWorkflow` | OperationalIntelligence | Preconditions e ExecutionSteps vazios |
| `GetServiceFinOps` | Governance | EfficiencyIndicators vazio, ReliabilityScore=0 |
| `ValidateContractIntegrity` | Catalog | Protobuf & GraphQL retornam stub |
...
**4 catch blocks vazios** em `CanonicalModelBuilder.cs` (OpenApi, Swagger, AsyncApi, Wsdl)

**12+ exceções silenciadas** sem logging em:
- `AsyncApiMetadataExtractor.cs` (3 instâncias)
- `WsdlMetadataExtractor.cs`, `WsdlSpecParser.cs` (3 instâncias)
- `OpenApiSpecParser.cs`, `SwaggerSpecParser.cs`, `AsyncApiSpecParser.cs` (6 instâncias)
- `ValidateContractIntegrity.cs` (1 instância)

**5 exceções que retornam null/false silenciosamente:**
- `TenantRepository.cs` (2), `RolePermissionRepository.cs` (1)
- `OllamaHttpClient.cs` (1), `AiDraftGeneratorService.cs` (1)
...
- ~30 imports/variáveis não utilizados
- 8 `any` types explícitos
- 6 `react-hooks/exhaustive-deps` warnings
- 2 `setState` em `useEffect`
...
- `ServiceDiscoveryPage` — 8 useQuery, 0 error states
- `DelegationPage` — 5 useQuery, 0 error states
- `AccessReviewPage` — 6 useQuery, 0 error states
...
- **⚠️ Password de dev (`ouro18`) em `appsettings.Development.json` com 24 connection strings**
...
### Backend
- **Total features:** ~550
- **Features com validators:** ~390 (70.7%)
- **Features sem validators:** ~160 (29.3%)
- **Handlers 100% stub:** 3
- **Handlers parcialmente stub:** 5+
- **Interfaces sem implementação:** 9 (4 com impacto funcional)
- **Catch blocks silenciosos:** 16+
