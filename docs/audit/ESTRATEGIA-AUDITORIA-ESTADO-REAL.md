...
| **Credenciais de BD desalinhadas em 2 jobs de CI** | `test-backend-e2e` arranca Postgres como `postgres`/`postgres` e não define nenhuma `ConnectionStrings__*`; as 9 strings do `appsettings.json` pedem `nextraceone` → `28P01` em 51 testes. `openapi-artifact` define **apenas** `ConnectionStrings__NexTraceOne`; as outras 8 mantêm o placeholder `Password=REPLACE_VIA_ENV`. O fallback de `GetRequiredConnectionString` não salva, porque só actua quando a string está *ausente* — e ela está *presente com placeholder*. Um placeholder derrota a salvaguarda desenhada para o cobrir. |
...
| **4 `Null*Repository` — investigados, não são bugs** | `NullAiAnalyticsRepository`, `NullAiSearchRepository`, `NullAiUsageEntryRepository`, `NullVectorStoreRepository`, todos em `aiknowledge`. O baseline sinalizou-os como candidatos a bug pela regra do CLAUDE.md §21. Lidos os XML docs, são **fallbacks de providers externos opcionais** (ClickHouse, Elasticsearch, vector store), não repositórios de persistência CRUD. A regra §21 não se aplica; o que existe é um problema de nomenclatura — são `Null*Provider` a usar o sufixo `Repository`, e é isso que faz a regra disparar em falso. |
| ~~**`notifications` sem migrations**~~ | **Refutado.** O módulo não tem DbContext próprio: as suas entidades foram consolidadas em `ConfigurationDbContext` (`ConfigurationDbContext.cs:115`), cujas migrations já as cobrem. Não é um gap. |
...
| ~~`analysis_v*.txt` (raiz)~~ | Análise endpoint↔handler por 12 módulos | Referiam caminhos inexistentes. **Removidos** — substituídos por `audit-baseline.sh`. |
