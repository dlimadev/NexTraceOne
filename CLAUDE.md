...
| Analytics | ClickHouse | provider analítico único |
...
| Observabilidade analítica | ClickHouse | provider único |
...
│       Dapper reporting, ClickHouse analytics writer, HTTP client configurável,
│       IDeadLetterRepository
│
├── BuildingBlocks.Observability
│   └── OpenTelemetry wiring (traces/métricas/logs), Serilog,
│       health checks, ClickHouse observability provider,
...
- **ClickHouse** — único provider analítico (Elasticsearch removido)
...
```bash
# Stack completa (Postgres, ClickHouse, Ollama, Nginx, apps)
...
- `docker-compose.production.yml` — produção (hardening, resource limits, ClickHouse)
...
| Elasticsearch (observabilidade/analytics) | ❌ Removido |
| ClickHouse (analytics) | ✅ Operacional — provider único |
