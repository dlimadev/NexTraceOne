...
/// </summary>
internal sealed class PromotionGateRepository(ChangeGovernanceDbContext context) : IPromotionGateRepository
{
    /// <inheritdoc />
    public async Task<PromotionGate?> GetByIdAsync(PromotionGateId id, CancellationToken cancellationToken = default)
        => await context.CiPromotionGates
    ...
    public async Task AddAsync(PromotionGate gate, CancellationToken cancellationToken = default)
        => await context.CiPromotionGates.AddAsync(gate, cancellationToken);
}
