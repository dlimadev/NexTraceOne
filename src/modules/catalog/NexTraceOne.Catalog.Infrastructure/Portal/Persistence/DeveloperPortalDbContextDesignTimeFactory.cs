...
/// </summary>
internal sealed class DeveloperPortalDbContextDesignTimeFactory : IDesignTimeDbContextFactory<DeveloperPortalDbContext>
{
    /// <summary>Cria instância do DeveloperPortalDbContext configurada para geração de migrations.</summary>
    public DeveloperPortalDbContext CreateDbContext(string[] args)
    {
        var optionsBuilder = new DbContextOptionsBuilder<DeveloperPortalDbContext>();
        var connectionString = Environment.GetEnvironmentVariable("NEXTRACEONE_CONNECTION_STRING")
            ?? "Host=localhost;Port=5432;Database=nextraceone_catalog;Username=nextraceone;Password=ouro18";

        optionsBuilder.UseNpgsql(connectionString,
            npgsql => npgsql.MigrationsAssembly(typeof(DeveloperPortalDbContext).Assembly.FullName));

        return new DeveloperPortalDbContext(
