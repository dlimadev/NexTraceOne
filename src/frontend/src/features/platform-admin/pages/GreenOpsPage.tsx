    ...
    <PageContainer>
      <div className="space-y-6">
        <PageHeader
          title={t('title')}
          subtitle={t('subtitle')}
          icon={<Leaf size={24} className="text-success" />}
          actions={
        ...
        {data && (
          <>
            ...
            {/* Top Services */}
            <section>
              <h2 className="text-lg font-medium text-heading mb-3">{t('topServicesTitle')}</h2>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-elevated border-b border-edge">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 text-muted font-medium">{t('colTeam')}</th>
                      <th className="text-left px-4 py-3 text-muted font-medium">{t('colEmissions')}</th>
                      <th className="text-left px-4 py-3 text-muted font-medium">{t('colTrend')}</th>
                      <th className="text-left px-4 py-3 text-muted font-medium">{t('colBar')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-edge/50">
                    {data.topServices.map((svc, idx) => {
                      ...
                      return (
                        <tr key={svc.serviceId} className="hover:bg-elevated">
                          <td className="px-4 py-3">
                            <span className="text-xs text-faded mr-2">#{idx + 1}</span>
                            <span className="font-medium text-heading">{svc.serviceName}</span>
                          </td>
                          <td className="px-4 py-3 text-muted">{svc.teamName}</td>
            ...
            {data.trend.length > 0 && (
              <section>
                <h2 className="text-lg font-medium text-heading mb-3">{t('trendTitle')}</h2>
                    ...
                    {data.trend.map((point) => {
                      ...
                      return (
                        <div key={point.month} className="flex-1 flex flex-col items-center gap-1">
                          <span className="text-xs text-muted">{point.totalKgCo2.toFixed(0)}</span>
                          <div
                            className="w-full bg-success rounded-t"
                            style={{ height: `${barHeight}%` }}
                            title={`${point.month}: ${point.totalKgCo2.toFixed(1)} kgCO₂`}
                          />
                          <span className="text-xs text-faded">{point.month.slice(0, 3)}</span>
                        </div>
                      );
                    })}
                    <div className="flex items-center gap-1 ml-2">
                      <Target size={12} className="text-warning" />
                      <span className="text-xs text-warning">{t('esgTarget')}</span>
                    </div>
                  </div>
                </div>
              </section>
...
function SummaryCard({
  ...
  value: string;
  subtitle?: string;
  color: 'body' | 'success' | 'warning' | 'accent';
}) {
  const colorMap = {
    body: 'text-body',
  ...
  return (
    <div className="border border-edge rounded-lg p-4 bg-card">
      ...
      {subtitle && <p className="text-xs text-faded mt-0.5">{subtitle}</p>}
    </div>
  );
}
...
function TrendBadge({ change }: { change: number }) {
  if (change > 0) {
    return (
      <span className="flex items-center gap-1 text-critical text-xs font-medium">
