...
function AuditHistoryPanel({ configKey }: { configKey: string }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        <p className="text-xs text-gray-400">{t('opsFinOpsConfig.audit.loading', 'Loading audit history...')}</p>
      </div>
    );
  }
  ...
  if (!audits || audits.length === 0) {
    return (
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
        <p className="text-xs text-gray-400">{t('opsFinOpsConfig.audit.empty', 'No audit history available.')}</p>
      </div>
    );
  }
