...
export function GovernanceConfigurationPage() {
  ...
  // Filter definitions to only governance-related ones
  const governanceDefinitions = useMemo(() => {
    if (!definitions) return [];
    return definitions.filter((d) => d.key.startsWith('governance.'));
  }, [definitions]);

  // Get definitions for the active section
  const sectionDefinitions = useMemo(() => {
    const section = SECTIONS.find((s) => s.key === activeSection);
    if (!section || !governanceDefinitions.length) return [];
    return governanceDefinitions.filter((d) =>
      section.prefixes.some((p) => d.key.startsWith(p))
    );
  }, [governanceDefinitions, activeSection]);
  ...
  if (definitionsError) {
    return (
      <PageContainer>
        <PageErrorState
          title={t('governanceConfig.error.title', 'Error loading configuration')}
          message={t(
            'governanceConfig.error.message',
            'Could not load governance and compliance configuration.'
          )}
          action={
            <Button
              variant="primary"
  ...
  return (
    <PageContainer>
      <PageHeader
        title={t('governanceConfig.title', 'Governance & Compliance Configuration')}
        subtitle={t(
          'governanceConfig.subtitle',
          'Configure policies, compliance profiles, evidence, waivers, governance packs, scorecards and risk matrix'
        )}
      />
      ...
      <div className="mb-6">
        <Tabs
          ...
          items={SECTIONS.map((section) => ({
            id: section.key,
            label: t(`governanceConfig.sections.${section.key}`, section.key),
            icon: section.icon,
          }))}
          activeId={activeSection}
          onChange={(id) => setActiveSection(id as GovernanceSection)}
        />
      </div>
        ...
        <CardBody>
          <div className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[200px]">
              <Select
                label={t('governanceConfig.scope', 'Scope')}
                value={scope}
            ...
            {scope !== 'System' && (
              <div className="flex-1 min-w-[200px]">
                <TextField
                  label={t('governanceConfig.scopeReference', 'Scope Reference ID')}
          ...
          <CardBody>
            <div className="text-center py-12 text-faded">
              ...
              <p className="text-sm mt-1">
                {t(
                  'governanceConfig.empty.description',
                  'No governance configuration definitions match the current filters.'
                )}
              </p>
            </div>
          </CardBody>
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {def.isSensitive && (
                          <Badge variant="warning">
                            ...
                            {t('governanceConfig.badges.sensitive', 'Sensitive')}
                          </Badge>
                        )}
                        {!def.isInheritable && (
                          <Badge variant="default">
                            {t('governanceConfig.badges.nonInheritable', 'Non-inheritable')}
                          </Badge>
                        )}
                      </div>
                      ...
                      {eff && !isEditing && (
                        <div className="mt-3 p-3 rounded-lg bg-subtle border border-edge">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {eff.isDefault && (
                              <Badge variant="default">
                                {t('governanceConfig.badges.default', 'Default')}
                              </Badge>
                            )}
                            {eff.isInherited && (
                              <Badge variant="info">
                                <Layers className="w-3 h-3 mr-1" />
                                {t('governanceConfig.badges.inherited', 'Inherited')}
                              </Badge>
                            )}
                            {!eff.isDefault && !eff.isInherited && (
                              <Badge variant="success">
                                {t('governanceConfig.badges.override', 'Override')}
                              </Badge>
                            )}
                          </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {def.isEditable && !isEditing && (
                        <IconButton
                          ...
                          label={t('governanceConfig.actions.edit', 'Edit')}
                      ...
                      <IconButton
                        ...
                        label={t('governanceConfig.actions.audit', 'Audit History')}
