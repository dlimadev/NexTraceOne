...
function AuditHistoryPanel({ configKey }: { configKey: string }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-3 pt-3 border-t border-edge">
        <p className="text-xs text-muted">{t('workflowConfig.audit.loading', 'Loading audit history...')}</p>
      </div>
    );
  }
  ...
  if (!audits || audits.length === 0) {
    return (
      <div className="mt-3 pt-3 border-t border-edge">
        <p className="text-xs text-muted">{t('workflowConfig.audit.empty', 'No audit history available.')}</p>
      </div>
    );
  }
