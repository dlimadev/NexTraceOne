          ...
          <PageSection>
            <Card>
              ...
              <CardBody className="p-0">
                {probes.length === 0 ? (
                  <div className="p-8 text-center text-muted text-sm">{t('syntheticMonitoring.noRecords')}</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {probes.map((p) => (
                          <tr key={p.id} className="border-b border-edge/50 hover:bg-muted/30 transition-colors">
                            <td className="px-4 py-2.5 font-medium">{p.name}</td>
                            <td className="px-4 py-2.5"><Badge variant="secondary">{t(`syntheticMonitoring.probeTypes.${p.type}`)}</Badge></td>
                            <td className="px-4 py-2.5 font-mono text-xs text-muted max-w-xs truncate" title={p.target}>{p.target}</td>
                            ...
                            <td className="px-4 py-2.5 text-xs text-muted">{p.lastResult}</td>
