          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            <Card>
              <CardBody className="text-center">
                ...
                <div className="text-xs text-muted">{t('phase5.compliance.i18nCoverage', 'i18n Coverage')}</div>
            ...
            <Card>
              <CardBody className="text-center">
                ...
                <div className="text-xs text-muted">{t('phase5.compliance.validationCoverage', 'Validation Coverage')}</div>
                <div className="text-xs text-faded mt-1">
            ...
            <Card>
              <CardBody className="text-center">
                ...
                <div className="text-2xl font-bold text-warning">{summary.deprecatedCount}</div>
                <div className="text-xs text-muted">{t('phase5.compliance.deprecated', 'Deprecated Parameters')}</div>
              </CardBody>
            </Card>
            <Card>
              <CardBody className="text-center">
                <Lock size={20} className="mx-auto mb-2 text-critical" />
                <div className="text-2xl font-bold">{summary.sensitiveCount}</div>
                <div className="text-xs text-muted">{t('phase5.compliance.sensitive', 'Sensitive Parameters')}</div>
              </CardBody>
            </Card>
          </div>
              ...
              <CardBody>
                <div className="space-y-2">
                  {summary.byCategory.map((cat) => {
                    ...
                    return (
                      <div key={cat.category} className="flex items-center justify-between py-2 border-b border-edge/40">
                        <div>
                          <span className="text-sm font-medium">{cat.category}</span>
                          <span className="text-xs text-faded ml-2">({cat.total} {t('phase5.compliance.params', 'params')})</span>
                        </div>
              ...
              <CardBody>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold text-success">{summary.editableCount}</span>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-lg font-bold text-muted">{summary.readOnlyCount}</span>
                  </div>
                </div>
                    ...
                    <div className="space-y-1 max-h-40 overflow-auto">
                      {summary.deprecatedKeys.map((key) => (
                        <div key={key} className="flex items-center gap-2 text-xs">
                          ...
                          <code className="text-body">{key}</code>
                        </div>
                      ))}
                    </div>
                  </div>
